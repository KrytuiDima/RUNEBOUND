...
export function computeSmoothnessScore(points) {
    if (points.length < 3)
        return 1.0;
    const angles = [];
...
export function computeClosureScore(points, isClosedForm, bboxDiagonal) {
    if (!isClosedForm)
        return 1.0;
    if (points.length < 2)
        return 0.0;
